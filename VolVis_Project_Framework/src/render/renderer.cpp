...
{
    glm::vec3 isoColor(0.0f, 1.0f, 1.0f); // Red color for the isosurface

    glm::vec3 samplePos = ray.origin + ray.tmin * ray.direction;
    const glm::vec3 increment = sampleStep * ray.direction;
    float lastSampleValue = m_pVolume->getSampleInterpolate(samplePos - increment);

    for (float t = ray.tmin; t <= ray.tmax; t += sampleStep, samplePos += increment) {
        float currentSampleValue = m_pVolume->getSampleInterpolate(samplePos);

        // Check if we crossed the isosurface threshold
        if ((lastSampleValue < m_config.isoValue && currentSampleValue >= m_config.isoValue) || (lastSampleValue >= m_config.isoValue && currentSampleValue < m_config.isoValue)) {
            // Refine intersection using bisection method
            float accurateT = bisectionAccuracy(ray, t - sampleStep, t, m_config.isoValue);
            glm::vec3 intersectionPoint = ray.origin + ray.direction * accurateT;

            if (!m_config.volumeShading) {
                // If volume shading is disabled, return the isosurface color
                return glm::vec4(isoColor, 1.0f);
            } else {
                // If volume shading is enabled, calculate Phong shading
                volume::GradientVoxel gradientVoxel = m_pGradientVolume->getGradient(intersectionPoint.x, intersectionPoint.y, intersectionPoint.z);
                glm::vec3 normal = glm::normalize(gradientVoxel.dir);
                glm::vec3 lightDir = glm::normalize(m_pCamera->position() - intersectionPoint);
                glm::vec3 viewDir = -glm::normalize(ray.direction);
                glm::vec3 shadedColor = computePhongShading(isoColor, gradientVoxel, lightDir, viewDir);
    ...
    // No intersection with the isosurface was found
...
float Renderer::bisectionAccuracy(const Ray& ray, float t0, float t1, float isoValue) const
{
    ...
    for (int i = 0; i < 10; ++i) { // Limit the iterations to avoid infinite loop
        midT = (t0 + t1) * 0.5f;
        float midValue = m_pVolume->getSampleInterpolate(ray.origin + midT * ray.direction);
        if (std::abs(midValue - isoValue) < 0.01) {
            break;
        }
        if ((midValue > isoValue && t0 > isoValue) || (midValue <= isoValue && t0 <= isoValue)) {
            t0 = midT;
        } else {
            t1 = midT;
        }
...
{// Extract the normal and make sure it's normalized
    glm::vec3 N = glm::normalize(gradient.dir);

    // Calculate ambient component
    float ka = 0.1f; // Ambient reflectivity
    glm::vec3 ambient = ka * color;

    // Calculate diffuse component
    float kd = 0.7f; // Diffuse reflectivity
    float NdotL = std::max(glm::dot(N, L), 0.0f);
    glm::vec3 diffuse = kd * NdotL * color;

    // Calculate specular component
    float ks = 0.2f; // Specular reflectivity
    glm::vec3 R = glm::reflect(-L, N);
    float RdotV = std::max(glm::dot(R, V), 0.0f);
    float alpha = 100.0f; // Shininess coefficient
    glm::vec3 specular = ks * pow(RdotV, alpha) * glm::vec3(1.0f, 1.0f, 1.0f); // Assuming white specular light
...
{
    glm::vec4 accumulatedColor(0.0f); // Initialize the accumulated color as transparent
    glm::vec3 samplePos = ray.origin + ray.tmin * ray.direction;
    const glm::vec3 increment = sampleStep * ray.direction;

    for (float t = ray.tmin; t <= ray.tmax; t += sampleStep, samplePos += increment) {
        float sampleValue = m_pVolume->getSampleInterpolate(samplePos);
        glm::vec4 color = getTFValue(sampleValue); // Get color and opacity from 1D transfer function

        // Apply Phong shading if volume shading is enabled
        if (m_config.volumeShading) {
            volume::GradientVoxel gradientVoxel = m_pGradientVolume->getGradientInterpolate(samplePos);
            glm::vec3 lightDir = glm::normalize(m_pCamera->position() - samplePos);
            glm::vec3 viewDir = -glm::normalize(ray.direction);
            glm::vec3 shadedColor = computePhongShading(color, gradientVoxel, lightDir, viewDir);
            color.r = shadedColor.r;
            color.g = shadedColor.g;
            color.b = shadedColor.b;
        }

        // Front-to-back compositing
        accumulatedColor += (1.0f - accumulatedColor.a) * color.a * color;
        accumulatedColor.a += (1.0f - accumulatedColor.a) * color.a;

        // Early ray termination if the accumulated color is opaque
