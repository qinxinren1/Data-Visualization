...
{
    // Get the base coordinates as integers
    int xBase = static_cast<int>(std::floor(coord.x));
    int yBase = static_cast<int>(std::floor(coord.y));
    int zBase = static_cast<int>(std::floor(coord.z));

    // Calculate the factors for interpolation
    float xFactor = coord.x - xBase;
    float yFactor = coord.y - yBase;
    float zFactor = coord.z - zBase;

    // Interpolate along x for the eight corners
    GradientVoxel c000 = getGradient(xBase, yBase, zBase);
    GradientVoxel c100 = getGradient(xBase + 1, yBase, zBase);
    GradientVoxel c001 = getGradient(xBase, yBase, zBase + 1);
    GradientVoxel c101 = getGradient(xBase + 1, yBase, zBase + 1);
    GradientVoxel c010 = getGradient(xBase, yBase + 1, zBase);
    GradientVoxel c110 = getGradient(xBase + 1, yBase + 1, zBase);
    GradientVoxel c011 = getGradient(xBase, yBase + 1, zBase + 1);
    GradientVoxel c111 = getGradient(xBase + 1, yBase + 1, zBase + 1);

    // Interpolate along y
    GradientVoxel c00 = linearInterpolate(c000, c100, xFactor);
    GradientVoxel c01 = linearInterpolate(c001, c101, xFactor);
    GradientVoxel c10 = linearInterpolate(c010, c110, xFactor);
    GradientVoxel c11 = linearInterpolate(c011, c111, xFactor);

    // Interpolate along z
    GradientVoxel c0 = linearInterpolate(c00, c01, zFactor);
    GradientVoxel c1 = linearInterpolate(c10, c11, zFactor);

    
    return linearInterpolate(c0, c1, yFactor);
}
...
{
    glm::vec3 dir = g0.dir * (1.0f - factor) + g1.dir * factor; // Interpolate the direction
    float magnitude = g0.magnitude * (1.0f - factor) + g1.magnitude * factor; // Interpolate the magnitude
