...
float Volume::getSampleTriLinearInterpolation(const glm::vec3& coord) const
{
    ...
    // Get the integer coordinates surrounding the interpolation point
    ...
    float xFactor = coord.x - x;
    float yFactor = coord.y - y;
    float zFactor = coord.z - z;
    
    // Interpolate on the xy plane for the bottom and top slices
    float bottom = biLinearInterpolate(glm::vec2(coord.x, coord.y), z);
    float top = biLinearInterpolate(glm::vec2(coord.x, coord.y), z + 1);
    
    // Interpolate between the two planes in z direction
    return linearInterpolate(bottom, top, zFactor);
}
...
float Volume::biLinearInterpolate(const glm::vec2& xyCoord, int z) const
{
    ...
    // Compute the factors for interpolation
    float xFactor = xyCoord.x - x;
    float yFactor = xyCoord.y - y;
    ...
    float i0 = linearInterpolate(v00, v10, xFactor);
    float i1 = linearInterpolate(v01, v11, xFactor);

    // Interpolate in y direction
    return linearInterpolate(i0, i1, yFactor);
}
